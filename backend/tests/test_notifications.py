...
def test_notification(db, test_user):
    notification = Notification(
        client_id=test_user.id, subject="Test Notification", message="This is a test notification", read=False, created_at=datetime.utcnow()
    )
    db.add(notification)
    db.commit()
...
def test_create_notification(authorized_client, test_user):
    response = authorized_client.post(
        "/notifications/", json={"subject": "New Test Notification", "message": "This is a test notification with email"}
    )
...
def test_mark_notification_as_read(authorized_client, test_notification, db):
    ...
    assert db_notification.read == True
...
def test_mark_all_notifications_as_read(authorized_client, db, test_user):
    ...
    unread_count = db.query(Notification).filter(Notification.client_id == test_user.id, Notification.read == False).count()
