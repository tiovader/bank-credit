...
def test_register_user(client, db):
    response = client.post("/auth/register", json={"email": "new_user@example.com", "password": "new_password"})
...
def test_register_existing_email(client, test_user):
    response = client.post(
        "/auth/register", json={"email": "test_user@example.com", "password": "another_password"}
    )
...
def test_login_success(client, test_user):
    response = client.post("/auth/token", data={"username": "test_user@example.com", "password": "test_password"})
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_login_wrong_password(client, test_user):
    response = client.post("/auth/token", data={"username": "test_user@example.com", "password": "wrong_password"})
